...
namespace MantisBT\Exception\Project\Category;
...
class CategoryNotFound extends ExceptionAbstract
{
    ...
    public function __construct($p_parameters = null, \Exception $p_previous = null)
    {
        if ($p_parameters === null) {
            $t_message = lang_get('exception_category_not_found_for_project');
        } else {
            $t_message = vsprintf( lang_get('exception_category_not_found_for_project'), $p_parameters);
        }
