...
/**
 * Database Field Not Found Exception
 * @package MantisBT
 * @subpackage classes
 */
class FieldNotFound extends ExceptionAbstract
{
    ...
    public function __construct($p_parameters = null, \Exception $p_previous = null)
    {
        if ($p_parameters === null) {
            $t_message = lang_get('exception_db_field_not_found');
        } else {
            $t_message = vsprintf( lang_get('exception_db_field_not_found'), $p_parameters);
        }
