...
/**
 * Tag Already Attached Exception
 * @package MantisBT
 * @subpackage classes
 */
class TagAlreadyAttached extends ExceptionAbstract
{
    ...
    public function __construct($p_parameters = null, \Exception $p_previous = null)
    {
        if ($p_parameters === null) {
            $t_message = _('That tag already attached to that issue.');
        } else {
            $t_message = vsprintf( _('That tag already attached to that issue.'), $p_parameters);
        }
