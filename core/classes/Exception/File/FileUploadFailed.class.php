...
class FileUploadFailed extends ExceptionAbstract
{
    ...
    public function __construct($p_parameters = null, \Exception $p_previous = null)
    {
        if ($p_parameters === null) {
            $t_message = _('File upload failed. File is not readable by MantisBT. Please check the project settings.');
        } else {
            $t_message = vsprintf( _('File upload failed. File is not readable by MantisBT. Please check the project settings.'), $p_parameters);
        }
