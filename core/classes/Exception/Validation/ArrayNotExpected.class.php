...
/**
 * Array Not Expected Exception
 * @package MantisBT
 * @subpackage classes
 */
class ArrayNotExpected extends ExceptionAbstract
{
    /**
     * Constructor
    ...
    public function __construct($p_parameters = null, \Exception $p_previous = null)
    {
        if ($p_parameters === null) {
            $t_message = _('A string was expected but an array was received for %1$s.');
        } else {
            $t_message = vsprintf( _('A string was expected but an array was received for %1$s.'), $p_parameters);
        }
