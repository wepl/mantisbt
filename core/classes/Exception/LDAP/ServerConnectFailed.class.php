...
/**
 * LDAP Server connection failed Exception
 * @package MantisBT
 * @subpackage classes
 */
class ServerConnectFailed extends ExceptionAbstract
{
    ...
    public function __construct($p_parameters = null, \Exception $p_previous = null)
    {
        if ($p_parameters === null) {
            $t_message = lang_get('exception_ldap_server_connect_failed');
        } else {
            $t_message = vsprintf( lang_get('exception_ldap_server_connect_failed'), $p_parameters);
        }
